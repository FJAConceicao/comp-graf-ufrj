...
function arc(center, radius, t1, t2, fill = true) {
  ...
  context.beginPath();
  context.arc(center[0], center[1], radius, arcInit, arcEnd, counterclockwise);
  //context.arc(center[0], center[1], radius, pi - arcInit, pi - arcEnd);
  if (fill) context.fill();
  else context.stroke();
}
...
async function readZones() {
  const requestURL = "./localtime.json"; //corrige local para nascer e por do sol
...
function drawClock(place) {
  ...
  drawClock.romans.map((n, i) => {
    ...
    // sinal de menos no i para inverter a ordem dos números romanos do relógio
  ...
  drawClock.decimals.map((n, i) => {
    ...
    // sinal de menos no i para inverter a ordem dos números do relógio
